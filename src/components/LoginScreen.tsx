      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="bg-white/80 backdrop-blur-md p-8 rounded-[32px] shadow-lg w-full max-w-sm border border-white/50">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-2">
            ...
            <AnimatePresence>
              {error && (<motion.div key="login-error" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="flex items-center gap-1.5 px-2 text-rose-500 overflow-hidden"><AlertCircle className="w-3.5 h-3.5" /><span className="text-xs font-bold tracking-wide">密碼錯誤，請重新輸入</span></motion.div>)}
            </AnimatePresence>
          </div>
