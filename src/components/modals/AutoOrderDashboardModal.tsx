...
import { X, ChevronLeft, ChevronRight, Edit2, CalendarDays, Bot, Search, MoreVertical } from 'lucide-react';
...
const getRelativeDayLabel = (dateStr: Date) => {
      ...
      >
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            <button
              type="button"
              disabled={syncingCustomerIds.has(customer.id)}
              onClick={async () => {
                setSyncingCustomerIds(prev => new Set(prev).add(customer.id));
                try {
                  await onToggleAutoOrder(customer.id);
                } finally {
              ...
              } ${syncingCustomerIds.has(customer.id) ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
            >
            ...
            <div className="relative">
              ...
              {activeDropdownId === customer.id && (
                <>
                  ...
                  />
                  <div className="absolute right-0 top-full mt-2 w-36 bg-white rounded-xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.1)] border border-slate-100 z-50 overflow-hidden py-1">
                    <button 
                      onClick={(e) => { e.stopPropagation(); onEditItems(customer); setActiveDropdownId(null); }}
                    ...
                    >
                      <Edit2 className="w-4 h-4 text-blue-500" /> 修改品項
                    </button>
                    <button 
                      onClick={(e) => { e.stopPropagation(); onSetHoliday(customer.id); setActiveDropdownId(null); }}
                      className="w-full text-left px-4 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50 flex items-center gap-2 transition-colors"
                    >
                      <CalendarDays className="w-4 h-4 text-orange-500" /> 設定公休
                    </button>
                  </div>
                </>
              )}
        ...
        {hasWarning && (
          <div className="mt-4 pt-4 border-t border-amber-100">
            <button 
              ...
              className="w-full flex items-center justify-center gap-2 py-2.5 bg-amber-50 hover:bg-amber-100 text-amber-700 active:bg-amber-200 rounded-xl text-sm font-bold transition-colors"
            >
              ⚠️ 立即設定預設品項
            </button>
          </div>
        ...
        <div className="p-4 sm:p-6 border-b flex flex-col sticky top-0 bg-white z-10 gap-4">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
            ...
            <div className="flex-1 sm:flex-none flex items-center justify-between bg-slate-50 rounded-xl p-1 border border-slate-100">
              ...
              <div className="px-2 sm:px-4 font-bold text-sm text-slate-700 sm:min-w-[120px] text-center flex items-baseline justify-center gap-1">
                {formatDateStr(previewDate)} 
                <span className="text-xs font-bold text-slate-400">
                  {getRelativeDayLabel(previewDate)}
                </span>
              </div>
