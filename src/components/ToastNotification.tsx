...
export const ToastNotification: React.FC<{ toast: Toast | null }> = ({ toast }) => {
  return (
    <AnimatePresence>
      {toast && (
        <motion.div key="toast-notification" initial={{ opacity: 0, y: -20, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -20, scale: 0.9 }} className="fixed top-6 inset-x-0 flex justify-center z-[200] pointer-events-none px-4">
